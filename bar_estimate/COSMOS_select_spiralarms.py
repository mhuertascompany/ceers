...
count_arms = merge['t5_is_there_any_spiral_arm_pattern__yes_pred'].values

#count_weak = merge['t4_is_there_a_bar__weak_bar_pred'].values
count_none = merge['t5_is_there_any_spiral_arm_pattern__no_pred'].values
...
max_bar_ids = np.where( (p_arms>0.5) & (p_feature>0.5) & (p_edgeon<0.5) & (q>0.5))[0]
print(max(m150))
...
max_bar_image_F200W.save("/n03data/huertas/COSMOS-Web/zoobot/bar_candidates/spirals_f150w.jpg")
