...
for f,tr in zip(filters,train):
    ...
    if os.path.exists(data_NGDEEP+'image_arrays/image_arrays_'+f+'.npz'):
        ...
        X_JWST,fullvec,idvec,fieldvec,ravec,decvec = read_NGDEEP_data(f,data_NGDEEP)

        if WRITE:
            print("writing image files for filter "+ str(f))
            np.savez(data_NGDEEP+'image_arrays/image_arrays_'+f+'.npz', stamps = X_JWST, fullvec = fullvec, idvec=idvec,fieldvec=fieldvec,ravec=ravec,decvec=decvec)
    ...
    for num in range(nruns):
        ...
        label_predictor.save_weights(data_NGDEEP+"models/JADES_adversarial_asinh_resnet_"+f+"vDR05_1122_shuffle_"+str(num)+".weights")
        feature_generator.save_weights(data_NGDEEP+"models/JADES_adversarial_asinh_resnet_"+f+"vDR05_1122_shuffle_"+str(num)+".weights")           
        chunk=1000
    ...
    if TRAIN:
        ...
        df.to_csv(data_NGDEEP+"cats/NGDEEP_adversarial_asinh_"+f+"_"+d4+"_4class_shuffle_"+str(nruns)+"_"+str(EPOCHS)+".csv")
