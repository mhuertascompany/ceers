...
def double_schechter_MCMC(smf_morph,path_out,filename,fit_range=(8.5,11.5)):
    ...
    file_path = os.path.join(path_out,filename+'_fit_results'+str(fit_range[0])+'.pkl')
    ...
    for (zbin, morph), results in fit_results.items():
        h5=os.path.join(samples_out_dir,filename+f'_{zbin}_{morph}_{fit_range[0]}_sampler.h5')
...
def schechter_MCMC(smf_morph,path_out,filename,fit_range=(8.5,11.5)):
...
for smf_type in smf_files:
    ...
    double_schechter_MCMC(smf_morph,path_in,smf_type,fit_range=(8.5,11.5))
