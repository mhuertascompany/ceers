...
queries = [
    ("morph_flag_f150w==0 and MAG_MODEL_F150W<24.5", 20,'sph'),  # Replace with your actual query and number of ids
    ("morph_flag_f150w==1 and MAG_MODEL_F150W<24.5", 20,'disk'),  # Replace with your actual query and number of ids
    ("morph_flag_f150w==2 and MAG_MODEL_F150W<24.5", 20,'irr'),
    ("morph_flag_f150w==3 and MAG_MODEL_F150W<24.5", 20,'bd')
...
with open("/n03data/huertas/COSMOS-Web/cutouts/generate_cutouts.sh", "w") as file:
    ...
    for query, n,f in queries:
        ...
        command = f"{base_command} --source_ids {source_ids_str} --output_path {output_path+f} --cutout_size {cutout_size}\n"
