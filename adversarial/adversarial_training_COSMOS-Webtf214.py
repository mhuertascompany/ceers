...
def get_network(image_size=32, num_classes=4):
    ...
    x = tf.keras.layers.RandomRotation(0.25)(inputs)
    x = tf.keras.layers.RandomFlip()(x)
    x = layers.Conv2D(16, (2, 2), strides=1, padding="same", activation="relu")(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(64, (2, 2), strides=2, padding="same", activation="relu")(x)
    x = layers.MaxPool2D((2, 2))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(128, (3, 3), strides=1, padding="same", activation="relu")(x)
    x = layers.MaxPool2D((2, 2))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(128, (2, 2), strides=2, padding="same", activation="relu")(x)
    x = layers.BatchNormalization()(x)
    x = layers.Conv2D(128, (2, 2), strides=2, padding="same", activation="relu")(x)
    x = layers.BatchNormalization()(x)
    x = layers.Flatten()(x)
...
class LabelPredictor(tf.keras.Model):
    def __init__(self):
        super().__init__()
    ...
    def call(self, feats, training=False):
        x = self.d1(feats)
        x = self.dr(x, training=training)
        return self.d2(x)  


class DomainPredictor(tf.keras.Model):
    def __init__(self):
        super().__init__()
...
def train_step(images, labels, images2, domains, alpha):
    with tf.GradientTape(persistent=True) as tape:
        # Forward pass
        features_label = feature_generator(images, training=True)
        l_predictions = label_predictor(features_label, training=True)

        features_domain = feature_generator(images2, training=True)
        d_predictions = domain_predictor(features_domain, training=True)
    ...
    # Apply gradients
    f_optimizer.apply_gradients(zip(f_gradients + l_gradients, feature_generator.trainable_variables + label_predictor.trainable_variables))
    d_optimizer.apply_gradients(zip([alpha * grad for grad in d_gradients], domain_predictor.trainable_variables))
