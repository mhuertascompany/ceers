...
for nir_f200,w_v,cat in zip(nir_f200_list,w,cats):
  cat["F200_AB"] = 2.5*(23-np.log10(cat.F200*1e-9))-48.6
  sel = cat.query('F200_AB<26')
  #print(cat)  
  for idn,ra,dec in zip(sel.ID,sel.RA,sel.DEC):
          try:
              position = SkyCoord(ra,dec,unit="deg")
            #print(ra,dec)
              stamp = Cutout2D(nir_f200['SCI'].data,position,64,wcs=w_v)
...
def get_network(image_size=32, num_classes=3):
  ...
  outputs = layers.GlobalAveragePooling2D()(bn5)
