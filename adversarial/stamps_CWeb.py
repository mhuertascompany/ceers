...
def plot_stamps_quantiles(wl,morph,ceers_cat,data_path,nquants_z=10,nquants_mass=4,quants_stamps_z=[0,3,6,9],quants_stamps_mass=[0,1,2,3]):
    ...
    with PdfPages(data_path+'figures/'+'morph_'+str(morph)+'_CWeb3.1_'+str(wl)+'_'+d4+'_Q.pdf') as pdf_ceers:
        ...
        sel = ceers_cat.query('((LP_MNUV_phys - LP_MR_phys) > 3 * (LP_MR_phys - LP_MJ_phys)) and ((LP_MNUV_phys - LP_MR_phys) > 3.1) and morph_flag=='+str(morph)+' and LP_zfinal>'+str(0.5)+' and LP_zfinal<'+str(7)+' and LP_mass_med_PDF<10 and  LP_mass_med_PDF>8 and MAG_MODEL_F444W<26')
