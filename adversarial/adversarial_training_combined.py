...
def read_CEERS_data(filter,data_path):
    cat_ceers =   pd.read_csv(data_path+"cats/CEERS_v0.51_photom.csv")
...
def read_JADES_data(filter,data_path):
    
    
    with fits.open(data_path+"JADES_DR2_PHOT_ZPHOT_PZETA_MASS.fits") as hdul:
    ...
    nir_f200 = fits.open(data_path+"images/hlsp_jades_jwst_nircam_goods-s-deep_"+filter+"_v2.0_drz.fits")
    ...
    sel = cat_jades.query('F200_AB<27 and F200_AB>0')
...
def read_NGDEEP_data(filter,data_path):
    
    
    with fits.open(data_path+"cats/ngdeep-full_photom_v0.21_pz.fits") as hdul:
        # Assuming the table you're interested in is in the first extension
        # This might need adjustment if your data is in a different HDU
        data = hdul[1].data
    ...
    sel = cat_ngdeep.query('F200_AB<27 and F200_AB>0')
    

    #print(cat)  
    for idn,ra,dec in zip(sel.ID, sel.RA,sel.DEC):
        try:
            full = 'ngdeep_'+str(idn)
            position = SkyCoord(ra,dec,unit="deg")
            field='ngdeep-combined'
            #print(ra,dec)
            stamp = Cutout2D(nir_f200['SCI'].data,position,32,wcs=w)
...
def create_datasets_combined(X_C, label_C, X_JWST_list, sh=True, n_JWST=5000):
    # Split the CANDELS data into training and testing sets
    train_s = len(X_C) * 4 // 5
    test_s = len(X_C) * 1 // 5
    ...
    with tf.device('CPU'):
        # CANDELS datasets
        CANDELS_X = tf.convert_to_tensor(X[:train_s], dtype=tf.float32)
        CANDELS_X = tf.expand_dims(CANDELS_X, -1)
        label_candels = tf.one_hot(label[:train_s], 4).numpy()

        CANDELS_X_t = tf.convert_to_tensor(X[train_s:train_s + test_s], dtype=tf.float32)
        CANDELS_X_t = tf.expand_dims(CANDELS_X_t, -1)
        label_candels_t = tf.one_hot(label[train_s:train_s + test_s], 4).numpy()

        # JWST combined dataset
        combined_samples = []
        for X_JWST in X_JWST_list:
            indices = np.random.choice(len(X_JWST), min(len(X_JWST), n_JWST), replace=False)
            sampled = np.array(X_JWST)[indices.astype(int)]
            combined_samples.append(sampled)
        JWST_X_sampled = np.concatenate(combined_samples)
        
        JWST_X = tf.convert_to_tensor(JWST_X_sampled, dtype=tf.float32)
        JWST_X = tf.expand_dims(JWST_X, -1)

        label_JWST = tf.one_hot(np.zeros(len(JWST_X)), 4).numpy()

        # Complete JWST datasets
        JWST_X_all = []
        for X_JWST in X_JWST_list:
            JWST_X_full = tf.convert_to_tensor(X_JWST, dtype=tf.float32)
            JWST_X_full = tf.expand_dims(JWST_X_full, -1)
            JWST_X_all.append(JWST_X_full)

    return CANDELS_X, label_candels, CANDELS_X_t, label_candels_t, JWST_X, label_JWST, JWST_X_all
...
data_JADES = "/n03data/huertas/JADES/"
data_NGDEEP = "/n03data/huertas/NGDEEP/"
data_CEERS = "/n03data/huertas/CEERS/"
data_c = "/n03data/huertas/JWST_deep_combined/"

data_paths = ["/n03data/huertas/NGDEEP/","/n03data/huertas/JADES/","/n03data/huertas/CEERS/"]

fields = ['NGDEEP','JADES','CEERS']
...
for f,tr in zip(filters,train):
    ...
    if os.path.exists(data_NGDEEP+'image_arrays/image_arrays_'+f+'.npz'):
        print("Loading saved array with data from filter "+f)
        data = np.load(data_NGDEEP+'image_arrays/image_arrays_'+f+'.npz',allow_pickle=True)
        # Access the saved variables
        X_JWST_ngdeep = data['stamps']
        fullvec_ngdeep = data['fullvec']
        idvec_ngdeep = data['idvec']
        fieldvec_ngdeep = data['fieldvec']
        ravec_ngdeep = data['ravec']
        decvec_ngdeep = data['decvec']
        
    else:

        X_JWST_ngdeep,fullvec_ngdeep,idvec_ngdeep,fieldvec_ngdeep,ravec_ngdeep,decvec_ngdeep = read_NGDEEP_data(f,data_NGDEEP)

        if WRITE:
            print("writing image files for filter "+ str(f))
            np.savez(data_NGDEEP+'image_arrays/image_arrays_'+f+'.npz', stamps = X_JWST_ngdeep, fullvec = fullvec_ngdeep, idvec=idvec_ngdeep,fieldvec=fieldvec_ngdeep,ravec=ravec_ngdeep,decvec=decvec_ngdeep)
    ...
    if os.path.exists(data_JADES+'image_arrays/image_arrays_'+f+'.npz'):
        ...
        # Access the saved variables
        X_JWST_jades = data['stamps']
        fullvec_jades = data['fullvec']
        idvec_jades = data['idvec']
        fieldvec_jades = data['fieldvec']
        ravec_jades = data['ravec']
        decvec_jades = data['decvec']
        
    else:

        X_JWST_jades,fullvec_jades,idvec_jades,fieldvec_jades,ravec_jades,decvec_jades = read_JADES_data(f,data_JADES)

        if WRITE:
            print("writing image files for filter "+ str(f))
            np.savez(data_JADES+'image_arrays/image_arrays_'+f+'.npz', stamps = X_JWST_jades, fullvec = fullvec_jades, idvec=idvec_jades,fieldvec=fieldvec_jades,ravec=ravec_jades,decvec=decvec_jades)

    if os.path.exists(data_CEERS+'image_arrays/image_arrays_'+f+'.npz'):
        print("Loading saved array with data from filter "+f)
        data = np.load(data_CEERS+'image_arrays/image_arrays_'+f+'.npz',allow_pickle=True)
        # Access the saved variables
        X_JWST_ceers = data['stamps']
        fullvec_ceers = data['fullvec']
        idvec_ceers = data['idvec']
        fieldvec_ceers = data['fieldvec']
        ravec_ceers = data['ravec']
        decvec_ceers = data['decvec']
        
    else:

        X_JWST_ceers,fullvec_ceers,idvec_ceers,fieldvec_ceers,ravec_ceers,decvec_ceers = read_CEERS_data(f,data_CEERS)

        if WRITE:
            print("writing image files for filter "+ str(f))
            np.savez(data_CEERS+'image_arrays/image_arrays_'+f+'.npz', stamps = X_JWST_ceers, fullvec = fullvec_ceers, idvec=idvec_ceers,fieldvec=fieldvec_ceers,ravec=ravec_ceers,decvec=decvec_ceers)

             

    fullvec_list=[fullvec_ngdeep,fullvec_jades,fullvec_ceers]
    idvec_list=[idvec_ngdeep,idvec_jades,idvec_ceers]
    fieldvec_list=[fieldvec_ngdeep,fieldvec_jades,fieldvec_ceers]
    ...
    for num in range(nruns):
        ...
        CANDELS_X,label_candels,CANDELS_X_t,label_candels_t,JWST_X,label_JWST,JWST_X_combined = create_datasets_combined(X,label,[X_JWST_ngdeep,X_JWST_jades,X_JWST_ceers])
        ...
        label_predictor.save_weights(data_c+"models/combined_adversarial_asinh_resnet_"+f+"vDR05_1122_shuffle_"+str(num)+".weights")
