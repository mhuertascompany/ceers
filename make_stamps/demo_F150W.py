...
def zero_pix_fraction(img):
    '''
    calculate the fraction of incomplete imaging
    '''
...
fit_flag = cat['F150W_FLAG'].values
star_flag = cat['star_flag'].values
Re = cat['F150W_RE'].values
axis_ratio = cat['F150W_Q'].values
...
for n in range(1,11):
    ...
    if n in POINTING2:
        img_name = "ceers_nircam%i_f150w_v0.51_i2d.fits"%n
            ...
            if (found[i]==0) & (fit_flag[i]==0) & (star_flag[i]==0):
                ...
                if all([up<xmax,down>-1,right<ymax,left>-1]):   
                    ...
                    if zero_pix_fraction(cut)<0.1:
                        ...
                        image.save('images/demo_F150W/F150W_%i.jpg'%i)
